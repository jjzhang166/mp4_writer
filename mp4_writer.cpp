...
public:
	...
	int open( const char* name, int w, int h, int frame, int sampler, int pernum, unsigned char *sps, int sps_size)
	{
		...
		0x64, //sps[5], //sps[1] AVCProfileIndication
		0x0, //sps[6], //sps[2] profile_compat
		0x20, //sps[7], //sps[3] AVCLevelIndication
		4); //3); // 4 bytes length before each NAL unit
...
public:
	...
	void write_video(int chn, unsigned char * data, size_t size, int frametype, unsigned long long ts )
	{
		...
		if( m_mp4 )
		{
			LOGI("m_mp4 not NULL, then write_video");
...
int main() {
	g_group[0].vi.cap_width = 1280;
	g_group[0].vi.cap_height = 720;
	g_group[0].vi.framerate = 1;
	//const char *file_path = env->GetStringUTFChars(path, 0);
	const char *file_path = "/sdcard/";
...
jint JNICALL Java_com_powervision_video_writer_AVCWriter_native_1write
  (JNIEnv *env, jobject thiz, write_mp4 *obj, jint channel, jbyteArray data, jlong size, jint frame_type, jlong ts) {
	...
	if(1) { //(pdata[4] & 0x1f) == 0x05) {	
		obj->write_video(channel, (unsigned char *)pdata, size, 1, ts);
	} 
	else 
	{
		obj->write_video(channel, (unsigned char *)pdata, size, 0, ts);
	}
